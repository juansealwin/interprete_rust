...
(defn interpretar [cod regs-de-act cont-prg pila mapa-regs]
  (let [fetched (cod cont-prg),
       ...
       (case opcode
          ...
          ; CHR: Incrementa cont-prg en 1, quita de la pila dos elementos (un string y un indice), 
          ; selecciona el char del string indicado por el indice y lo coloca al final de la pila.
          CHR (let [primero (last pila),
                    segundo (last (butlast pila)),
                    cadena (if (string? primero) primero segundo), 
                    indice (if (string? primero) segundo primero),
                    nueva-pila (vec (drop-last 2 pila))] 
                    (recur cod regs-de-act (inc cont-prg) (conj nueva-pila (nth cadena indice)) mapa-regs))

          ; FLUSH: Purga la salida e incrementa cont-prg en 1.
          FLUSH (do (flush) (recur cod regs-de-act (inc cont-prg) pila mapa-regs)) 

          ;CAL: Llamada a una funcion. Agrega al final de regs-de-act el reg-de-act (proveniente de mapa-regs) 
          ; indicado por el argumento, cambia cont-prg por el valor del argumento y coloca al final de la pila 
          ...
          JC (let [elem (last pila), 
                  ...
                  (if (= elem true)
                    (recur cod regs-de-act (second fetched) nueva-pila mapa-regs)  
          ...
          ; RETN: Indica el retorno de la llamada a un procedimiento (no funcion). Llama recursivamente a 
          ; interpretar con valores actualizados de regs-de-act (se elimina el ultimo de ellos), 
          ; cont-prg (pasa a ser el ultimo valor en la pila) y pila (se quita de ella el nuevo cont-prg).
          RETN (let [ultimo-pila (last pila),
                    nueva-pila (vec (butlast pila))]
                    (recur cod (vec (butlast regs-de-act)) ultimo-pila nueva-pila mapa-regs))

          ; Incrementa cont-prg en 1 y quita el ultimo elemento de pila. El argumento indica donde sumar 
          ; el elemento en el ultimo de los regs-de-act al llamar recursivamente a interpretar 
          ; (verificando la compatibilidad de los tipos)
          ; Por ejemplo: 
          ; fetched: [POPADD 2]
          ...
          ; en reg-actual las coordenadas [#reg-act, offset] donde sumar el elemento en regs-de-act 
          ; al llamar recursivamente a interpretar (verificando la compatibilidad de los tipos)
          ; Por ejemplo: 
          ; fetched: [POPADDREF 2]
          ...
          ; NEG: Incrementa cont-prg en 1, quita de la pila un elemento numerico, le cambia el signo y lo coloca al 
          ; final de la pila.
          NEG (let [elemento (- (last pila)),
                    nueva-pila (conj (vec (butlast pila)) elemento)] 
                    (recur cod regs-de-act (inc cont-prg) nueva-pila mapa-regs))

          ; NOT: Incrementa cont-prg en 1, quita de la pila un elemento booleano, lo niega y lo coloca al final de la pila.
          NOT (let [elemento (not (last pila)),
                    nueva-pila (conj (vec (butlast pila)) elemento)] 
                    (recur cod regs-de-act (inc cont-prg) nueva-pila mapa-regs))   
          ...
          SQRT (let [elemento (Math/sqrt (last pila)),
                    nueva-pila (conj (vec (butlast pila)) elemento)] 
                    (recur cod regs-de-act (inc cont-prg) nueva-pila mapa-regs))  
          
          ; SIN: Incrementa cont-prg en 1, quita de la pila un elemento numerico, calcula su seno y lo coloca al 
          ; final de la pila.
          SIN (let [elemento (Math/sin (last pila)),
                    nueva-pila (conj (vec (butlast pila)) elemento)] 
                    (recur cod regs-de-act (inc cont-prg) nueva-pila mapa-regs))  
          
          ; ATAN: Incrementa cont-prg en 1, quita de la pila un elemento numerico, calcula su arcotangente y 
          ; la coloca al final de la pila.
          ATAN (let [elemento (Math/atan (last pila)),
                    nueva-pila (conj (vec (butlast pila)) elemento)] 
                    (recur cod regs-de-act (inc cont-prg) nueva-pila mapa-regs))            

          ; ABS: Incrementa cont-prg en 1, quita de la pila un elemento numerico, calcula su valor absoluto y 
          ; lo coloca al final de la pila.
...
(defn ins-salto-y-tabulador [idx tokens nro-bloque]
  (let [token (nth tokens idx)]
    (cond 
      (= (symbol ";") token)
        (concat 
          (concat (take (inc idx) tokens) (concat (list \newline) (repeat nro-bloque \tab)))
      ...
      :else 
        (concat 
          (concat (take idx tokens) (concat (list \newline) (repeat nro-bloque \tab)))
          (concat 
            (concat (list token) (concat (list \newline) (repeat (inc nro-bloque) \tab)))
            (nthrest tokens (inc idx)))
        )
    )
  )
  ...
  ([idx tokens nro-bloque] (
    (if (> (count tokens) idx)
      ...
      (cond
        (= (symbol "{") token) 
          (listar 
            (+ idx 3 nro-bloque (inc nro-bloque)) 
...
(defn dump [instrucciones] 
  (cond 
    (= instrucciones nil) (println "0 nil") 
    :else (println(
            clojure.string/join 
              \newline 
              (map-indexed #(str % \space %2) instrucciones)
    ))
  ) 
)
...
(defn ya-declarado-localmente? [identificador contxt] 
  (
    > 
    (reduce + (
      map (fn [elem] 
        (cond
          (= (elem 0)  identificador) 1
          :else 0
        )
      )
      (subvec (contxt 1) (last (contxt 0)))
    )) 
    0        
  )
)
        ...
        assoc ambiente 4 (
          ...
          (
            ...
            (
              conj 
              (conj 
                (vector (nth (reverse (simb-ya-parseados ambiente)) 4)) 
                (conj (vector 'const) (nth (reverse (simb-ya-parseados ambiente)) 2))
              ) 
              (nth (reverse (simb-ya-parseados ambiente)) 0)
            )
          )
        )
      )
...
(defn inicializar-contexto-local [ambiente]
  (if (= (estado ambiente) :sin-errores)
    (
      assoc ambiente 4 (conj (vector ( conj ((contexto ambiente) 0) (count ((contexto ambiente) 1)))) ((contexto ambiente) 1))
    )
    ambiente
  )
)
  ...
  (if (= (estado ambiente) :sin-errores)
    (
      assoc ambiente 4 (
        conj ( 
          conj 
            (vector (pop ((contexto ambiente) 0))) 
            (subvec ((contexto ambiente) 1) 0 (last ((contexto ambiente) 0))) 
        )
      )
    )
    ambiente
...
(defn buscar-tipo-de-retorno [ambiente dir] 
  (
    let [res (filterv (fn [x] (= (x 2) dir)) ((contexto ambiente) 1))]

    (cond
      (= [] res) nil
      :else (last (last ((res 0) 1)))
    )
  )
)
...
(defn fixup [ambiente ubicacion]
  (if (= (estado ambiente) :sin-errores)
    (
      assoc ambiente 6 
        (assoc (bytecode ambiente) ubicacion 
          (assoc ((bytecode ambiente) ubicacion) 1 (count (bytecode ambiente))))
    )
    ambiente
  )
)  
...
(defn dividir [n1 n2]
  (cond
    (and (integer? n1) (integer? n2)) (quot n1 n2)
    :else (/ n1 n2)
  )
)
...
; true
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
(defn compatibles? [tipo val]
  (
    cond (vector? val) true
    :else (
      case tipo
        i64 (integer? val) 
        f64 (float? val)
...
(defn pasar-a-int [elemento]
  (cond
    (string? elemento) 
      (if (reduce #(and %1 %2) (map #(and (>= %1 48) (>= 57 %1)) (map int elemento))) 
        (let [n (read-string elemento)] (if (number? n) n elemento)) 
        elemento)
    (number? elemento) (int elemento)
    :else elemento
  )
)
...
(defn pasar-a-float [elemento]
  (cond
    (string? elemento) 
      (if (reduce #(and %1 %2) (map #(or (and (>= %1 48) (>= 57 %1)) (= %1 46)) (map int elemento))) 
        (let [n (read-string elemento)] (float (if (number? n) n elemento))) 
        elemento)
    (number? elemento) (float elemento)
    :else elemento
  )
)
...
;                                                   ^^^ ^
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
(defn cargar-en-ult-reg [vec dir tipo val]
  (assoc vec (dec (count vec)) (assoc (last vec) dir (vector tipo val)))
)
